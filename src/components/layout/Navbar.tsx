...
const Navbar = ({ onOpenCart, onToggleSidebar }: { onOpenCart: () => void, onToggleSidebar: () => void }) => {
  ...
  return (
    <header 
      className={cn(
        "fixed top-0 w-full z-50 px-4 md:px-8 h-20 flex justify-between items-center transition-all duration-500 font-sans selection:bg-brand-orange selection:text-white",
        scrolled 
          ? "bg-brand-black/80 backdrop-blur-xl border-b border-white/5 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)]" 
    ...
    >
      ...
      <div className="flex items-center gap-4 md:gap-6">
        ...
        <Link to="/" className="flex items-center gap-3 group outline-none">
          <div className="w-8 h-8 md:w-10 md:h-10 bg-brand-orange/10 rounded-xl flex items-center justify-center text-brand-orange shadow-inner border border-brand-orange/20 group-hover:bg-brand-orange group-hover:text-white transition-all duration-300">
            <Zap size={18} className="md:w-5 md:h-5" strokeWidth={2} />
          </div>
          <span className="font-display font-black text-xl md:text-2xl uppercase italic tracking-tight text-white group-hover:opacity-80 transition-opacity">
        ...
        >
          ...
          {totalItems > 0 && (
            <motion.div 
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="absolute -top-2 -right-2 bg-brand-orange text-white text-[10px] font-black px-1.5 py-0.5 rounded-full min-w-[1.25rem] h-5 flex items-center justify-center border-2 border-brand-black shadow-[0_0_10px_rgba(249,115,22,0.5)]"
            >
              {totalItems}
            </motion.div>
          )}
